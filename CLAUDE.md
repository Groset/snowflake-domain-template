...
| `sql/<db>/<schema>/...` | All DDL files for this domain. Below schema, organize however suits the domain — flat, by feature, by category, by purpose. The deploy assembler classifies by filename prefix (`SP_`, `UDF_`, `vw_`, `SEED_`) and CREATE statement, not by folder. |
| `grants/` | Grants on objects this repo owns (separate deploy role concern, kept at repo root) |
| `deploy-prd/` | **Gitignored.** Generated by the AI assembler when preparing a PRD deploy. Transient artifact — regenerate each time. |
...
**No automated deploy.** Two paths, depending on target environment.

### DEV (direct)

Source files are **DEV-hardcoded** — every database reference says `DEV_*`. Developers run them directly:

1. Open the `.sql` file in VSCode.
2. Confirm the connection is `DEV_<DOMAIN>` (role = `DAGSTER_DEV_ROLE`).
3. Click ▶.

Recommended manual run order when running multiple files: tables → functions → views → procedures → grants.

### PRD (AI-assembled)

PRD deploys go through an assembly step. An AI agent (Claude or similar) reads the source, substitutes `DEV_` → `PRD_`, bundles by deploy role and process step, and writes `deploy-prd/<role>/{01_definitions.sql, 02_seed.sql, 03_grants.sql, README.md}`. The deployer then runs the bundled scripts under the named role.

Full process and AI instructions: [`ai/context/deployment.md`](ai/context/deployment.md). Ask an AI agent to "prepare a PRD deploy" or "follow `ai/context/deployment.md`" to trigger assembly.

The assembled `deploy-prd/` tree is **gitignored** — regenerated each deploy. Source files in `sql/` are the only thing that gets committed.

### DDL patterns (same in both environments)
...
2. After merge: connect to DEV (`DAGSTER_DEV_ROLE`), run all changed source files in the recommended order.
3. PRD deployment: ask an AI agent to assemble a PRD deploy (see `ai/context/deployment.md`); review the generated `deploy-prd/` bundle; run each role's bundle in order in PRD. PRD deployment is a release step, distinct from day-to-day development.
...
1. Capture current DDL from Snowflake:
   ...
   ```
2. Save into `sql/<db>/<schema>/...` (organize however suits the domain below schema). Use `DEV_*` for every database reference inside the file body.
...
- **Procedure / function / view**: drop the `.sql` file under `sql/<db>/<schema>/` (anywhere below schema), run it via VSCode against DEV.
- **New table (default)**: drop a `CREATE OR REPLACE TABLE` file under `sql/<db>/<schema>/`, run it.
...
- **New seed-data script**: prefix the filename `SEED_*`. Use idempotent `MERGE` or guarded inserts. Goes under the same `sql/<db>/<schema>/` tree.
...
- **All references use `DEV_*`.** The assembler handles PRD substitution. Never hardcode `PRD_*` in source files.
